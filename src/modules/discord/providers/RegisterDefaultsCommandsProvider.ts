...
import HelpCommandRunner from '../runners/HelpCommandRunner';

@injectable()
class RegisterDefaultsCommandsProvider {
  public async execute(): Promise<void> {
    log.info(
      '[RegisterDefaultsCommandsProvider] Starting to register default commands',
    );
    ...
    try {
      ...
      commandoClient.registry.registerCommands([
        new HelpCommandRunner(commandoClient),
      ]);

      log.info(
        '[RegisterDefaultsCommandsProvider] Finished register default commands',
      );
    } catch (e) {
