...
class CustomCategoryRunner extends Commando.Command {
  ...
  async run(
    ...
    const [categoryName] = msg.content
      .toLowerCase()
      .replace('!', '')
      .split(' ');
    ...
    log.info(
      `command ${categoryName} is not allowed in discord server name: ${msg.guild.name}`,
    );
  ...
  isCategoryFromThisServer(categoryName: string, discord_id: string): boolean {
    ...
    return (
      guildRegisterCommands && guildRegisterCommands.includes(categoryName)
    );
  }
  ...
  createEmbedMessage(
    ...
    embed.setDescription(
      category.customCommands.map(customCommand => {
        return `**!${
          customCommand.name
        }** - _ ${customCommand.description.replace(/_/g, ' ')}_ `;
      }),
    );
